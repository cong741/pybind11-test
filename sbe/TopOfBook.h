...
#ifndef _SBE_TOPOFBOOK_CXX_H_
#define _SBE_TOPOFBOOK_CXX_H_
...
public:
    static const std::uint16_t SBE_BLOCK_LENGTH = static_cast<std::uint16_t>(185);
    ...
    TopOfBook(char *buffer, const std::uint64_t bufferLength) :
        TopOfBook(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    TopOfBook(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        TopOfBook(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }

    SBE_NODISCARD static SBE_CONSTEXPR std::uint16_t sbeBlockLength() SBE_NOEXCEPT
    {
        return static_cast<std::uint16_t>(185);
    }
    ...
    TopOfBook &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    {
        TopOfBook skipper(m_buffer, m_offset, m_bufferLength, sbeBlockLength(), m_actingVersion);
        skipper.skip();
    ...
    TopOfBook &sourceName(const std::uint64_t index, const char value)
    {
        if (index >= 16)
        {
    ...
    TopOfBook &putSourceName(const char *const src) SBE_NOEXCEPT
    {
    ...
    #if __cplusplus >= 201703L
    TopOfBook &putSourceName(const std::string_view str)
    {
    ...
    #else
    TopOfBook &putSourceName(const std::string &str)
    {
        const std::size_t srcLength = str.length();
        if (srcLength > 16)
        {
    ...
    TopOfBook &sourceSessionID(const std::uint64_t index, const std::uint8_t value)
    {
        if (index >= 16)
        {
    ...
    TopOfBook &putSourceSessionID(const char *const src) SBE_NOEXCEPT
    {
    ...
    TopOfBook &sourceSeqNo(const std::uint64_t value) SBE_NOEXCEPT
    {
    ...
    TopOfBook &originCreateTimeUTC(const std::int64_t value) SBE_NOEXCEPT
    {
    ...
    TopOfBook &exchange(const std::uint64_t index, const char value)
    {
        if (index >= 32)
        {
    ...
    TopOfBook &putExchange(const char *const src) SBE_NOEXCEPT
    {
    ...
    #if __cplusplus >= 201703L
    TopOfBook &putExchange(const std::string_view str)
    {
        const std::size_t srcLength = str.length();
        if (srcLength > 32)
        {
    ...
    #else
    TopOfBook &putExchange(const std::string &str)
    {
        const std::size_t srcLength = str.length();
        if (srcLength > 32)
        {
    ...
    TopOfBook &symbol(const std::uint64_t index, const char value)
    {
        if (index >= 48)
        {
    ...
    TopOfBook &putSymbol(const char *const src) SBE_NOEXCEPT
    {
    ...
    #if __cplusplus >= 201703L
    TopOfBook &putSymbol(const std::string_view str)
    {
        const std::size_t srcLength = str.length();
        if (srcLength > 48)
        {
    ...
    #else
    TopOfBook &putSymbol(const std::string &str)
    {
        const std::size_t srcLength = str.length();
        if (srcLength > 48)
        {
    ...
    SBE_NODISCARD static const char *layerMetaAttribute(const MetaAttribute metaAttribute) SBE_NOEXCEPT
    {
        switch (metaAttribute)
        {
            case MetaAttribute::PRESENCE: return "required";
    ...
    static SBE_CONSTEXPR std::uint16_t layerId() SBE_NOEXCEPT
    {
        return 202;
    }

    SBE_NODISCARD static SBE_CONSTEXPR std::uint64_t layerSinceVersion() SBE_NOEXCEPT
    {
        return 0;
    }

    SBE_NODISCARD bool layerInActingVersion() SBE_NOEXCEPT
    {
        return true;
    }

    SBE_NODISCARD static SBE_CONSTEXPR std::size_t layerEncodingOffset() SBE_NOEXCEPT
    {
        return 128;
    }
...
public:
    SBE_NODISCARD Layer &layer()
    {
        m_layer.wrap(m_buffer, m_offset + 128, m_actingVersion, m_bufferLength);
        return m_layer;
    }
    ...
    TopOfBook &updateType(const UpdateType::Value value) SBE_NOEXCEPT
    {
        std::uint8_t val = (value);
    ...
    {
        std::int64_t val;
        std::memcpy(&val, m_buffer + m_offset + 161, sizeof(std::int64_t));
        return SBE_LITTLE_ENDIAN_ENCODE_64(val);
    }

    TopOfBook &exchangeTimeUTC(const std::int64_t value) SBE_NOEXCEPT
    {
        std::int64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
        std::memcpy(m_buffer + m_offset + 161, &val, sizeof(std::int64_t));
        return *this;
    }
    ...
    {
        std::uint64_t val;
        std::memcpy(&val, m_buffer + m_offset + 169, sizeof(std::uint64_t));
        return SBE_LITTLE_ENDIAN_ENCODE_64(val);
    }

    TopOfBook &exchangeSequence(const std::uint64_t value) SBE_NOEXCEPT
    {
        std::uint64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
        std::memcpy(m_buffer + m_offset + 169, &val, sizeof(std::uint64_t));
        return *this;
    }
    ...
    {
        std::int64_t val;
        std::memcpy(&val, m_buffer + m_offset + 177, sizeof(std::int64_t));
        return SBE_LITTLE_ENDIAN_ENCODE_64(val);
    }

    TopOfBook &sendingTimeUTC(const std::int64_t value) SBE_NOEXCEPT
    {
        std::int64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
        std::memcpy(m_buffer + m_offset + 177, &val, sizeof(std::int64_t));
        return *this;
    }

template<typename CharT, typename Traits>
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const TopOfBook &_writer)
{
    TopOfBook writer(
        _writer.m_buffer,
        _writer.m_offset,
